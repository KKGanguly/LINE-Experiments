...
from optimizers.BOHBOptimizerKDE import BOHBOptimizerKDE
from optimizers.DefaultOptimizer import DefaultOptimizer
from optimizers.FAREOptimizerEasy import FAREOptimizerExp
...
def get_preprocessor(dataset):
    ...
    if 'holdout' in dataset and not dataset.get('disable') and dataset['task']=='defect prediction':
        return defect.preprocess
    elif 'cross_validation' in dataset and not dataset.get('disable') and dataset['task']=='issue lifetime prediction':
            ...
            for data_name, _ in train_files.items():
                ...
                for opt_cfg in optimizers:
                    ...
                    for checkpoint in checkpoints:
                        ...
                        if optimizer:
                            ...
                            for i in range(repeats):
                                ...
                                if run_optimizer:
                                    ...
                                    optimizer.optimize()
                                    try:
                                        elapsed_time = optimizer.measured_time
                                    except:
                        ...
                        if optimizer_name != 'FARE' and optimizer_name != 'BOHB': run_optimizer = False

if __name__ == "__main__":
    config_file = os.path.join("experiments", "final_configs", "config_BOHB_Issue.yaml")
